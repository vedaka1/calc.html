function getName() {
    ...
    if (birthDate.slice(0, 4) < today.getFullYear()) birthDate = today.getFullYear()+1 + birthDate.slice(4);
    birthDate = new Date(birthDate.replace(/^(\d+)\.(\d+).(\d+)$/, "$3-$2-$1"));
    document.getElementById('result_date_field').innerHTML = (Math.floor( (birthDate - today) / 1000 / 60 / 60 / 24)); // количество дней)
    ...
    switch(true) {
        ...
        break;
        case (birthMonth >= 3) && (birthMonth < 7):season.innerHTML += '<br/>' + 'Ты родился ' + birthDay + ', чудесный весенний день';
        break;
        case (birthMonth >= 7) && (birthMonth < 10):season.innerHTML += '<br/>' + 'Ты родился ' + birthDay + ', чудесный летний день';
        break;
        case (birthMonth >= 10) && (birthMonth < 12):season.innerHTML += '<br/>' + 'Ты родился ' + birthDay + ', чудесный осенний день';
        break;
    }
}